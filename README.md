...
Please Note: prompt & model matching system is still in development, and is not perfect at the moment. In addition, the model list is limited to 1000 items to reduce fetching time and prevent API quota limitation errors, and to reduce search time. Try prompts such as "What can you tell me about [(part of) model name]?", "What are the most significant features of [(part of) model name]"? "Is there such a model as [(part of) model name]? etc. Also Qwen/Qwen2.5-1.5B-Instruct is a small model, so even if a match is found, it may hallucinate, but it should then hallucinate around the fetched data.
